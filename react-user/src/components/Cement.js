...
const Cement = () => {
    ...
    // var bodyLotionProducts = st.filter(product => product.type.includes('bodyLotion'));

    // var bodyScrubProducts = st.filter(produt => produt.type.includes("bodyScrub"));

    // var bodyWashProducts = st.filter(product => product.type.includes("bodywash"));

    // var under40Products = st.filter(product => product.price > 16 && product.price < 40)
    ...
    return (
        <div className='u20MainParent fof '>
            ...
            <div className='u20HeadingHold gap-20 justify-center relative flex flex-col'>
                <p className='u20Heading'> CEMENT </p>
                <p className='u20Desc'> Sold Foundations Lasting Impressions</p>
            </div>
